...
def pgd_attack(model, data, target, epsilon=0.15, num_iter=40, alpha=0.015, random_start=True):
    if random_start:
        # 随机初始化扰动
        perturbed_data = data + torch.empty_like(data).uniform_(-epsilon, epsilon)
        perturbed_data = torch.clamp(perturbed_data, -3.0, 3.0)
    else:
...
def train_dae(dae, cnn, train_loader, val_loader, device):
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_loss * 0.999:
            best_loss = val_loss
            torch.save(dae.state_dict(), "best_dae_PGD.pth")
            early_stop_counter = 0
        else:
...
def main():
    ...
    def create_dae_dataset(subset):
        ...
        for inputs, labels, snrs in loader:
            ...
            # 使用PGD攻击替换BIM攻击
            adv = pgd_attack(cnn, inputs, labels, epsilon=0.15, num_iter=40, alpha=0.015, random_start=True)  
    ...
    plt.plot(snr_values, avg_acc['adv'], 'r--s', label='PGD Attack', linewidth=2, markersize=6)
    plt.plot(snr_values, avg_acc['dae'], 'g-.^', label='CEAE Defense', linewidth=2, markersize=6)
    
    plt.title('PGD Attacks CNN Classifier (RML2016.10a dataset)', fontsize=20)
