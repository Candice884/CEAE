...
def fgsm_attack(model, data, target, epsilon):
    ...
    perturbed_data = torch.clamp(perturbed_data, -3, 3)  # 限制扰动范围
...
def train_dae(dae, cnn, train_loader, val_loader, device):
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_loss * 0.999:
            best_loss = val_loss
            torch.save(dae.state_dict(), "best_dae_FGSM.pth")  # 使用新文件名
            early_stop_counter = 0
        else:
...
def main():
    ...
    def create_dae_dataset(subset):
        ...
        for inputs, labels, snrs in loader:
            ...
            adv = fgsm_attack(cnn, inputs, labels, epsilon=0.15)
    ...
    plt.plot(snr_values, avg_acc['adv'], 'r--s', label='FGSM Attack', linewidth=2, markersize=6)
    plt.plot(snr_values, avg_acc['dae'], 'g-.^', label='CEAE Defense', linewidth=2, markersize=6)
    
    plt.title('FGSM Attacks CNN Classifier (RML2016.10a dataset)', fontsize=20)
